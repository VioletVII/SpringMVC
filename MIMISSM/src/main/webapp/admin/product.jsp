                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${p.pId},${info.pageNum})">编辑 <%--,${info.pageNum}--%>
                                    </button>
...
alert(str+"11111111");
    ...
    function deleteBatch(page) {
        //得到所有选中复选框的对象，根据其长度判断是否有选中的商品
        var allChecked=$("input[name='ck']:checked");
        //如果有选中的商品，获取其value值，进行字符串的拼接
        if(allChecked.length==0){
            //说明没有商品被选中，提醒用户选择商品删除
            alert("请选择您要删除的商品");
        }else {
            //判断用户是否要删除，避免误删
            if (confirm("您确定要删除"+allChecked.length+"条商品吗？")){

                var str=""; //拼接的字符串
                var pid=""; //每一个商品的id
                //进行字符串的拼接
                $.each(allChecked,function (){
                    pid=$(this).val();//获取到每一个商品的id
                    str+=pid+",";
                });
                //alert(str);
